...
SELECT *,(nota_crite/med)*MED100  as prok,get_greutate_min(ID_PRODUS,1) as gmin;
 FROM &pbd WHERE comanda0>0 ORDER BY prok DESC INTO CURSOR CC

IF delta<0
?"PING------------------------------"
	SELECT *,IIF(comanda0+ROUND((DELTA*PROK/100)/gmin,0)<0,-comanda0,ROUND((DELTA*PROK/100)/gmin,0)) AS ROT FROM CC ORDER BY PROK DESC INTO CURSOR C2
